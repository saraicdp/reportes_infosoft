<?php
	...
	mysql_connect("localhost", "root", "police");
	...
	while ($row=mysql_fetch_array($rec))
	{
		...
		$sql="SELECT SUM(tot_can) AS totalcompra, SUM(tot_net) AS totalneto, SUM(pag_iva) AS iva FROM ventas WHERE fec_emi BETWEEN '$desde' AND '$hasta'";
		$rec=mysql_query($sql);
	while ($row=mysql_fetch_array($rec)){
