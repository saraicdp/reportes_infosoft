<?php
	...
	//Mostrar tabla
		...
		$pdf->cell(22,5,"C.I./ Rif",1,0,'C');
		$pdf->cell(80,5,"Razon Social",1,0,'C');
		$pdf->cell(20,5,"Base Imp.",1,0,'C');
	...
	$sql = "SELECT compras.num_com, compras.fec_emi, compras.num_fac, compras.ref_com,  compras.cod_pro, prove.nom_pro, prove.rif, compras.tot_net, compras.pag_iva, compras.tot_can 
	FROM compras 
	INNER JOIN prove 
	ON compras.cod_pro = prove.cod_pro 
	WHERE fec_emi 
	BETWEEN '$desde' 
	AND '$hasta' 
	order by fec_emi";
	...
	while ($row=mysql_fetch_array($rec))
	{
		...
		$pdf->cell(15,5,$row['num_fac'],1,0,'D');
		$pdf->cell(20,5,$row['ref_com'],1,0,'D');
		$pdf->cell(22,5,$row['rif'],1,0,'D');
		$pdf->cell(80,5,$row['nom_pro'],1,0,'D');
		$pdf->settextcolor(10,26,198);
		$pdf->SetFillColor(205,205,205);
		$pdf->cell(20,5,$row['tot_net'],1,0,'R','true');
		$pdf->cell(20,5,$row['pag_iva'],1,0,'R','true');
		$pdf->cell(20,5,$row['tot_can'],1,1,'R','true');
		$pdf->SetTextColor(0,0,0);
	...
	while ($row=mysql_fetch_array($rec)){
       	...
       	$pdf->Setmargins(239,5);
		$pdf->Ln();
		$pdf->SetFont("Arial", "", "8");
		$pdf->cell(20,5,"Iva:",1,0,'L');
		$pdf->cell(32,5,$row['iva'],1,1,'R');
		$pdf->cell(20,5,"Neto:",1,0,'L');
		$pdf->cell(32,5,$row['totalneto'],1,1,'R');
		$pdf->cell(20,5,"Total:",1,0,'L');
		$pdf->cell(32,5,$row['totalcompra'],1,1,'R');
